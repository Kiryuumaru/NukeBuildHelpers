...
internal class GithubPipeline(BaseNukeBuildHelpers nukeBuild) : IPipeline
{
    ...
    public void BuildWorkflow(BaseNukeBuildHelpers baseNukeBuildHelpers, AllEntry allEntry)
    {
        ...
        AddJobStepNukeRun(preSetupJob, RunnerOS.Ubuntu2204, "PipelinePreSetup", id: "NUKE_RUN", args: Id);
        ...
        {
            var testJob = AddJob(workflow, entryDefinition.Id, "Test - ${{ needs.pre_setup.nuke_entry_name }}", "${{ needs.pre_setup.nuke_runs_on }}", needs: [.. needs], _if: "success()");
            AddJobOrStepEnvVarFromNeeds(testJob, "NUKE_PRE_SETUP_OUTPUT", "pre_setup");
            AddJobStepCheckout(testJob, _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            //AddJobStepsFromBuilder(testJob, workflowBuilders, (wb, step) => wb.WorkflowBuilderPreTestRun(step));
            var cacheTestStep = AddJobStep(testJob, name: "Cache Test", uses: "actions/cache@v4", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            AddJobStepWith(cacheTestStep, "path", "./.nuke/cache");
            AddJobStepWith(cacheTestStep, "key", $$$"""
                test-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-${{ needs.pre_setup.nuke_run_classification }}-${{ needs.pre_setup.nuke_run_identifier }}"
                """);
            AddJobStepWith(cacheTestStep, "restore-keys", $$$"""
                test-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-${{ needs.pre_setup.nuke_run_classification }}-
                test-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-main-
                """);
            var nukeTestStep = AddJobStepNukeRun(testJob, "${{ needs.pre_setup.nuke_run_script }}", "PipelineTest", "${{ needs.pre_setup.nuke_entry_ids_to_run }}", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
        ...
        {
            var buildJob = AddJob(workflow, entryDefinition.Id, "Build - ${{ needs.pre_setup.nuke_entry_name }}", "${{ needs.pre_setup.nuke_runs_on }}", needs: [.. testNeeds], _if: "success()");
            AddJobOrStepEnvVarFromNeeds(buildJob, "NUKE_PRE_SETUP_OUTPUT", "pre_setup");
            AddJobStepCheckout(buildJob, _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            //AddJobStepsFromBuilder(buildJob, workflowBuilders, (wb, step) => wb.WorkflowBuilderPreBuildRun(step));
            var cacheBuildStep = AddJobStep(buildJob, name: "Cache Build", uses: "actions/cache@v4", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            AddJobStepWith(cacheBuildStep, "path", "./.nuke/cache");
            AddJobStepWith(cacheBuildStep, "key", $$$"""
                build-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-${{ needs.pre_setup.nuke_run_classification }}-${{ needs.pre_setup.nuke_run_identifier }}"
                """);
            AddJobStepWith(cacheBuildStep, "restore-keys", $$$"""
                build-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-${{ needs.pre_setup.nuke_run_classification }}-
                build-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-main-
                """);
            var nukeBuild = AddJobStepNukeRun(buildJob, "${{ needs.pre_setup.nuke_run_script }}", "PipelineBuild", "${{ needs.pre_setup.nuke_entry_ids_to_run }}", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            //AddJobStepsFromBuilder(buildJob, workflowBuilders, (wb, step) => wb.WorkflowBuilderPostBuildRun(step));
            var uploadBuildStep = AddJobStep(buildJob, name: "Upload Artifacts", uses: "actions/upload-artifact@v4", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            AddJobStepWith(uploadBuildStep, "name", "${{ needs.pre_setup.nuke_entry_id }}");
        ...
        {
            var publishJob = AddJob(workflow, entryDefinition.Id, "Publish - ${{ needs.pre_setup.nuke_entry_name }}", "${{ needs.pre_setup.nuke_runs_on }}", needs: [.. buildNeeds], _if: "success()");
            AddJobOrStepEnvVarFromNeeds(publishJob, "NUKE_PRE_SETUP_OUTPUT", "pre_setup");
            AddJobStepCheckout(publishJob, _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            var downloadBuildStep = AddJobStep(publishJob, name: "Download artifacts", uses: "actions/download-artifact@v4", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            AddJobStepWith(downloadBuildStep, "path", "./.nuke/output");
            AddJobStepWith(downloadBuildStep, "pattern", "${{ needs.pre_setup.nuke_entry_id }}");
            ...
            var cachePublishStep = AddJobStep(publishJob, name: "Cache Publish", uses: "actions/cache@v4", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
            AddJobStepWith(cachePublishStep, "path", "./.nuke/cache");
            AddJobStepWith(cachePublishStep, "key", $$$"""
                publish-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-${{ needs.pre_setup.nuke_run_classification }}-${{ needs.pre_setup.nuke_run_identifier }}"
                """);
            AddJobStepWith(cachePublishStep, "restore-keys", $$$"""
                publish-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-${{ needs.pre_setup.nuke_run_classification }}-
                publish-${{ needs.pre_setup.nuke_runner_name }}-${{ needs.pre_setup.nuke_entry_id }}-${{ needs.pre_setup.nuke_cache_invalidator }}-${{ needs.pre_setup.nuke_environment }}-main-
                """);
            var nukePublishTask = AddJobStepNukeRun(publishJob, "${{ needs.pre_setup.nuke_run_script }}", "PipelinePublish", "${{ needs.pre_setup.nuke_entry_ids_to_run }}", _if: "${{ needs.pre_setup.nuke_entry_id != 'skip' }}");
